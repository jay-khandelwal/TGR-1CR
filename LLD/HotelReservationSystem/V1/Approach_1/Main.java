package LLD.HotelReservationSystem.V1.Approach_1;
...
// HotelManagementService class (Singleton) for staff operations
class HotelManagementService {
    ...
    public List<Room> findAvailableRooms(RoomType roomType, LocalDate fromDate, LocalDate toDate) throws InvalidBookingException {
        ...
        return rooms.stream()
                .filter(room -> room.getRoomType().equals(roomType) && isRoomAvailable(room, fromDate, toDate))
    ...
    public Booking createBooking(Staff staff, Guest guest, RoomType roomType, LocalDate fromDate, LocalDate toDate)
        ...
        List<Room> availableRooms = findAvailableRooms(roomType, fromDate, toDate);
...
public class Main {
    public static void main(String[] args) {
        HotelManagementService hms = HotelManagementService.getInstance();
